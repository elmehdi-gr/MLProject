...
Since the project structure has been reorganized, please note the following:

> **IMPORTANT:** You MUST update file paths in the notebooks before running them.

- Change paths like `DATA_DIR = Path('processed_data')` to `DATA_DIR = Path('../data/processed')`
- Change paths like `DATA_DIR = Path('DatasetHAR')` to `DATA_DIR = Path('../data/raw')`
...
- Check `report/project_report.pdf` for the detailed findings.
